...
고정 오버레이, 화면 높이를 채우는 `320px` 패널과 `52px` 메뉴 행을
표현한다.
...
- 그림자와 애니메이션은 추가하지 않는다.
- 패널 바깥에는 `rgba(0, 0, 0, 0.8)` 딤드를 적용한다.
...
- [x] **3단계: LNB 마크업 구현**
  ...
  최초에는 선택 행이 없고 클릭한 링크만 선택 상태와 `aria-current`를 갖는다.

- [x] **4단계: 피그마 CSS 구현**

  패널 `320px × 100dvh`, 내부 여백 `8px`, 행 높이 `52px`, 행 좌우 여백
  `16px`, 둥글기 `16px`, 글자 `16px/24px/500`, 글자색 `#222222`, 선택
  배경 `#F6F8FB`, 딤드 `rgba(0, 0, 0, 0.8)`를 적용한다. `1100px`
  이상에서는 오버레이를 숨긴다. `1100px`부터 `1439px`까지는 헤더 좌우
  여백을 `20px`, 메뉴 간격을 `16px`로 줄여 오버플로를 방지한다.
